...
create or replace function public.move_todo_between_sections(
  p_todo_id uuid,
...
begin
  ...
  if p_target_section = 'today' then
    ...
    set
      today_date = case
        when todo.id = p_todo_id then (current_timestamp at time zone 'America/Toronto')::date
        else todo.today_date
      end,
