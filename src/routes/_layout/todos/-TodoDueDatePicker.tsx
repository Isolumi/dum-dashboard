...
export function TodoDueDatePicker({
  ...
  useEffect(() => {
    const next = getTodoDueDateInputValues(value);
  ...
  const displayValue = useMemo(() => {
    ...
    return formatTodoDueDate(toTodoDueDate(dateValue, timeValue));
  }, [dateValue, timeValue]);
  ...
  function updateDueDate(nextDateValue: string, nextTimeValue: string) {
    ...
    onChange(toTodoDueDate(nextDateValue, nextTimeValue));
  }
      ...
      <PopoverTrigger
        render={
          <Button
            ...
            aria-label={label}
            disabled={disabled}
            className="group/date min-h-11 w-full justify-end px-2 text-right text-inherit hover:text-inherit"
          />
        }
      >
        {displayValue ? (
          <span className={cn(compact ? "text-xs" : "text-sm")}>{displayValue}</span>
        ) : (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                if (!dateValue) return;
                onChange(toTodoDueDate(dateValue, nextTimeValue));
              }}
            />
          </div>
