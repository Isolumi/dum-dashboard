...
function podLogUrl(namespace: string, pod: string, container: string): URL {
  ...
  url.search = new URLSearchParams({ container }).toString();
  url.hash = "";
  return url;
}
...
export const proxyPodLogs = createServerOnlyFn(
  async (request: Request, namespace: string, pod: string): Promise<Response> => {
    ...
    try {
      const response = await fetch(podLogUrl(namespace, pod, container), {
