...
**Domain:** Personal dashboard / tool aggregator (React, full-stack, realtime)
**Researched:** 2026-03-28
**Confidence:** HIGH (all core technologies verified against official docs and current release pages)
...
| `pnpm`       | Package manager                 | Recommended for shadcn/ui CLI (`pnpm dlx shadcn@latest`). Faster installs, strict dependency resolution. All official TanStack Start examples use pnpm.                                                          |
...
pnpm create tanstack --template react-start
...
pnpm add @supabase/supabase-js @tanstack/react-query zod tw-animate-css

# 4. Supporting shadcn components (add as needed)
pnpm dlx shadcn@latest add button card sidebar badge
...
**`.oxlintrc.json` for a React + TypeScript project:**
...
Note: Supabase is transitioning from `ANON_KEY` to `PUBLISHABLE_KEY`. Both work currently; prefer `PUBLISHABLE_KEY` for new projects.
...
**For data that changes infrequently (e.g., todo list initial load):**
...
**For data that must stay live (e.g., todo items synced in realtime):**
...
**For new tools added to the dashboard:**
...
_Stack research for: Personal dashboard (TanStack Start + Tailwind v4 + shadcn/ui + Lucide + OXC + Supabase)_
